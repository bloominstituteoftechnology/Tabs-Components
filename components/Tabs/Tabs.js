class Tabs {
  constructor(element) {
    this.element = element; // references the full tab element, which contains tabs and content
    // create a reference to the ".tabs-link" class nested in your tab object
    this.links = this.element.querySelectorAll('.tabs-link') // links a nodeList of all tab links
    ...
    this.links = Array.from(this.links).map( link => {
      return new TabsLink(link, this); //nodeList is turned into an Array, then each individual link is used as a property of a new TabLink constructor; this refers to the Tabs Class
    });
...
class TabsLink {
  ...
  constructor(link, parent) {
    ...
    this.tabsItem = parent.getTab(this.link.dataset.tab); // the parameter is the value of the dataset 
