...
class TabLink {
  ...
  select() {
    ...
    Array.from(links).forEach( coollink => {
      coollink.classList.remove('tabs-link-selected');
...
class TabItem {
  ...
  select() {
    ...
    const tabbyItems = document.querySelectorAll('.tabs-item');
    // Remove the class "tabs-item-selected" from each element
    Array.from(tabbyItems).forEach(itemEle => itemEle.classList.remove('tabs-item-selected'));
    // Add a class named "tabs-item-selected" to this element 
    this.element.classList.add('tabs-item-selected'); //do i need the element keyword? 
