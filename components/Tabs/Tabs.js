class Tabs {
  constructor(theEntireTabElement) {
    this.theEntireTabElement = theEntireTabElement;
    // create a reference to the ".tabs-link" class nested in your tab object
    this.tabLinks = this.theEntireTabElement.querySelectorAll('.tabs-link');
  ...
  getTab(data) {
    ...
    return this.theEntireTabElement.querySelector(`.tabs-item[data-tab="${data}"]`);
  }
...
class TabsLink {
  ...
  constructor(link, parent) {
    ...
    this.tabsItem = parent.getTab(link.dataset.tab);
...
class TabsItem {
  constructor(tabsItemElement) {
    this.tabsItemElement = tabsItemElement;
  }

  select() {
    // Add a class named "tabs-item-selected" to the element 
    this.tabsItemElement.classList.add('tabs-item-selected');
  }

  deselect() {
    // Remove a class named "tabs-item-selected" from the element 
    this.tabsItemElement.classList.remove('tabs-item-selected');
